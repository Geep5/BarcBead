...
var BarcNostrClient = class {
  ...
  async sendMessage(content, url = null) {
    ...
    const event = await createEvent(
      ...
      content,
      [["r", channelId]]
      // 'r' tag = URL reference (plaintext, human-readable)
    );
    let published = false;
    for (const relay of connectedRelays) {
      if (relay.publish(event)) {
...
// src/lib/nostr-enhanced.js
var EnhancedNostrClient = class extends BarcNostrClient {
  constructor() {
    super();
  ...
  async joinRoom(roomContext) {
    ...
    const filters = [{
      kinds: [42, 1],
      // Chat messages and text notes
      limit: 100,
      since: Math.floor(Date.now() / 1e3) - 7 * 24 * 60 * 60
      // 1 week
    }];
    if (website) {
      filters[0]["#r"] = [website];
      this.currentChannelId = website;
    }
    if (users.length > 0) {
      filters[0]["#p"] = users;
    }
  ...
  async sendEnhancedMessage(content, options = {}) {
    const {
      ...
      mentions = [],
      // Additional user mentions in the message
      replyTo = null,
    ...
    const allMentions = [.../* @__PURE__ */ new Set([...mentions, ...parsedMentions])];
    const tags = [];
    if (website) {
      tags.push(["r", website]);
    }
    ...
    for (const relay of this.relays) {
      ...
      relay.subscribe(subId, filters, (event) => {
        if (strict && users.length > 0) {
          const taggedUsers = event.tags.filter((t) => t[0] === "p").map((t) => t[1]);
...
async function initClient(privateKey = null) {
  ...
  setInterval(checkConnectionStatus, 5e3);
  return nostrClient;
}
...
async function broadcastToAll(message) {
  ...
  chrome.runtime.sendMessage(message).catch((err2) => {
    console.log("[Background] Broadcast error (normal if no listeners):", err2?.message);
  });
}
...
async function handleMessage(request, sender) {
  switch (request.type) {
    ...
    case "SEND_DM": {
      ...
      const event = await nostrClient.sendEnhancedMessage(request.content, {
        users: [request.targetPubkey],
        // Create a room with just this user
        mentions: []
      });
      return { success: !!event, eventId: event?.id };
    }
